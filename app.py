...
round_choice = st.radio(
    "Tour analysé",
    ["Premier tour", "Second tour", "Programmes économiques"],
    horizontal=True,
)
...
if round_choice == "Premier tour":
    ...
    with st.sidebar:
        st.title("Filtres — Programmes économiques")
        candidats_disponibles = mesures["candidat"].drop_duplicates().tolist()
        candidats_choisis = st.multiselect(
            "Candidats",
            candidats_disponibles,
            default=candidats_disponibles,
        )
        categories = st.multiselect(
            "Catégories",
            sorted(mesures["categorie"].dropna().unique()),
            default=sorted(mesures["categorie"].dropna().unique()),
        )

    st.subheader("Programmes économiques et faisabilité")
    st.warning(
        "Les programmes 2027 ne sont pas tous stabilisés. "
        "Les mesures anciennes ou non confirmées sont signalées explicitement."
    )

    filtered = mesures[
        mesures["candidat"].isin(candidats_choisis)
        & mesures["categorie"].isin(categories)
    ].copy()
    ...
    with tabs_e[0]:
        st.dataframe(
            filtered[
                [
                    "candidat", "mesure", "categorie", "sens_budgetaire",
                    "statut_2027", "precision_chiffrage",
                    "financement_identifie", "niveau_confiance"
                ]
            ],
            width="stretch",
            hide_index=True,
        ...
        candidat_focus = st.selectbox(
            "Voir une fiche détaillée",
            filtered["candidat"].drop_duplicates().tolist()
            if not filtered.empty else [],
        )
        if candidat_focus:
            fiches = filtered[filtered["candidat"] == candidat_focus]
            mesure_focus = st.selectbox("Mesure", fiches["mesure"].tolist())
            fiche = fiches[fiches["mesure"] == mesure_focus].iloc[0]
            c1, c2, c3, c4 = st.columns(4)
            c1.metric("Sens budgétaire", fiche["sens_budgetaire"])
            c2.metric("Précision", fiche["precision_chiffrage"])
            c3.metric("Financement", fiche["financement_identifie"])
            c4.metric("Statut 2027", fiche["statut_2027"])
            st.markdown(f"**Réalité économique :** {fiche['realite_economique']}")
            st.markdown(f"**Réalité politique :** {fiche['realite_politique']}")
            st.markdown(f"**Véhicule juridique :** {fiche['vehicule_juridique']}")
            st.markdown(f"**Contrainte européenne :** {fiche['contrainte_europeenne']}")
            st.markdown(f"[Source officielle]({fiche['source_officielle']})")

    with tabs_e[1]:
        chart = filtered.dropna(
            subset=["montant_externe_bas_mde", "montant_externe_haut_mde"],
            how="all"
        ).copy()

        if chart.empty:
            st.info("Aucun ordre de grandeur externe exploitable pour les filtres choisis.")
        else:
            chart["milieu_estimation_mde"] = (
                chart["montant_externe_bas_mde"].fillna(chart["montant_externe_haut_mde"])
                + chart["montant_externe_haut_mde"].fillna(chart["montant_externe_bas_mde"])
            ) / 2
            fig_e = px.bar(
                chart.sort_values("milieu_estimation_mde"),
                x="milieu_estimation_mde",
                y="mesure",
                color="candidat",
                orientation="h",
                labels={
                    "milieu_estimation_mde": "Ordre de grandeur annuel (Md€)",
                    "mesure": "",
                    "candidat": "",
                },
            )
            fig_e.update_layout(height=540, legend_title_text="")
            st.plotly_chart(fig_e, width="stretch")
            st.caption(
                "Le graphique utilise le milieu de la fourchette externe. "
                "Il ne s'agit ni d'un coût officiel définitif ni d'une prévision."
            )

        st.dataframe(
            filtered[
                [
                    "candidat", "mesure", "sens_budgetaire",
                    "montant_annonce_mde", "montant_externe_bas_mde",
                    "montant_externe_haut_mde", "financement_annonce"
                ]
            ],
            width="stretch",
            hide_index=True,
    ...
    with tabs_e[2]:
        feasibility = filtered[
            [
                "candidat", "mesure", "vehicule_juridique",
                "contrainte_europeenne", "contrainte_politique",
                "financement_identifie", "precision_chiffrage"
            ]
        ]
        st.dataframe(feasibility, width="stretch", hide_index=True)
        st.info(
            "Les appréciations de faisabilité décrivent les obstacles institutionnels "
            "et budgétaires connus. Elles ne constituent pas une note politique."
        )

    with tabs_e[3]:
        st.subheader("Point de départ macro-budgétaire")
        cols = st.columns(len(contexte))
        for idx, (_, row) in enumerate(contexte.iterrows()):
            cols[idx].metric(row["indicateur"], f"{row['valeur']} {row['unite']}")
        st.dataframe(contexte, width="stretch", hide_index=True)
        st.markdown(
            "Une mesure nouvelle doit être lue à partir de ce point de départ : "
            "déficit élevé, dette élevée et marges budgétaires réduites."
        )

    with tabs_e[4]:
        st.dataframe(filtered, width="stretch", hide_index=True)
        st.download_button(
            "Télécharger les mesures économiques",
            filtered.to_csv(index=False).encode("utf-8-sig"),
            "mesures_economiques_2027.csv",
            "text/csv",
        )
